    ...
    <div style="overflow-x:auto;">
      <div style="min-width:940px;">
        <div style="display:grid; grid-template-columns:34px minmax(0,1fr) 128px 158px 150px 96px 126px; gap:10px; padding:11px 14px; background:#101017; border-bottom:1px solid #1f1f2b; font-size:10.5px; font-weight:700; letter-spacing:.06em; color:#8b899f;">
          <div><input v-if="canEdit" type="checkbox" :checked="allSelected" @change="$emit('toggle-select-all')" style="width:13px; height:13px; accent-color:#7c6fff; cursor:pointer;" /></div>
          <div>TAREFA &amp; DESCRIÇÃO</div><div>CARGO</div><div>RESPONSÁVEL</div><div>PRAZO</div><div>PRIORIDADE</div><div>STATUS</div>
        </div>

        <div v-if="loading" style="padding:24px 14px; font-size:12.5px; color:#8b899f; display:flex; align-items:center; gap:8px;"><i class="fi fi-sr-hourglass"></i>Carregando tarefas…</div>
        ...
        <TransitionGroup v-else tag="div" @enter="listEnter" @leave="listLeave" :css="false">
          <div v-for="(t, i) in tasks" :key="t.id" :data-index="i" @click="openTask(t)"
            ...
            <div @click.stop><input v-if="canEdit" type="checkbox" :checked="isSelected(t.id)" @change="$emit('toggle-select', t.id)" style="width:15px; height:15px; accent-color:#7c6fff; cursor:pointer;" /></div>
            <div style="min-width:0;">
              <div style="display:flex; align-items:center; gap:8px;">
                <span style="font-family:'JetBrains Mono', monospace; font-size:10.5px; color:#6f6d87;">{{ t.code }}</span>
                ...
                <span v-if="t.subtasks_total" style="font-size:10.5px; color:#8b899f; flex:none;"><i class="fi fi-sr-check-circle" style="opacity:.6;"></i> {{ t.subtasks_done }}/{{ t.subtasks_total }}</span>
                <span v-if="t.attachments_total" style="font-size:10.5px; color:#8b899f; flex:none;"><i class="fi fi-sr-paperclip" style="opacity:.6;"></i> {{ t.attachments_total }}</span>
              </div>
            ...
            <div :style="{ fontSize: '11.5px', fontWeight: '600', color: isLate(t) ? '#ff8f98' : '#9a97b8', display: 'flex', alignItems: 'center', gap: '6px' }">
              <i class="fi fi-sr-calendar" style="font-size:10.5px; opacity:.75;"></i>{{ formatDue(t) }}
            </div>
